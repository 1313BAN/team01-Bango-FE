...
// 선택된 값
const selectedSido = ref<string | null>(null);
...
const handleDongChange = (dongName: string) => {
  ...
  console.log("최종 선택된 동 코드:", dongName);
};
...
const searchHandler = async () => {
  const dongcode = dongList.value.find(
    (item) =>
      ...
      item.sidoName === selectedSido.value
  )?.dongCode;
  ...
  if (dongcode != null) {
    ...
    console.log(await fetchHouseInfo(sggCd, umdCd, "2024", "10"));
  }
};
