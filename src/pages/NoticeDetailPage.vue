<template>
  <div :id="id">
    <!-- 상단 헤더 + 버튼 -->
    <header
      class="flex items-center justify-between mx-auto max-w-2xl px-4 py-5"
    >
    ...
    <div
      class="max-w-2xl mx-auto p-4 border rounded-lg shadow bg-white overflow-auto max-h-[90vh] flex flex-col mb-6"
    >
      <!-- 로딩 / 에러 -->
            ...
            <div class="flex-1 space-y-1">
              <div class="flex items-center justify-between">
                <h1 class="text-xl font-semibold text-gray-900">
                  {{ rentalNotice.noticeName }}
                </h1>
                <span
                  :class="
                    rentalNotice.isLiked
              ...
              <div class="flex flex-wrap gap-1 text-xs text-gray-600">
                <span
                  :class="`px-2 py-0.5 rounded-full ${statusColorClass(
                    rentalNotice
                  )}`"
                >
                ...
                <span
                  class="px-2 py-0.5 bg-green-100 text-green-800 rounded-full"
                >
                  종류: {{ rentalNotice.status || "-" }}
                </span>
                <span
                  class="px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full"
                >
            ...
            <table class="w-full text-sm border rounded overflow-hidden">
              <tbody>
                <tr class="border-b">
                  ...
                  <td class="p-2">
                    {{ formatDate(rentalNotice.announceDate) || "-" }}
                  </td>
                </tr>
                <tr class="border-b">
                  <th scope="row" class="bg-gray-50 p-2">접수기간</th>
                  <td class="p-2">
                    {{ formatDate(rentalNotice.beginDate) || "-" }} ~
                    {{ formatDate(rentalNotice.endDate) || "-" }}
                  </td>
                </tr>
                <tr>
              ...
              <tbody>
                ...
                <tr class="border-b">
                  ...
                  <td class="p-2" colspan="3">
                    {{ rentalNotice.sidoName || "-" }}
                    {{ rentalNotice.gugunName || "" }}
                  </td>
                </tr>
                <tr class="border-b">
      ...
      <template v-else>
        <p class="text-center text-gray-500 flex-grow">
          공고 정보를 불러오는 중입니다...
        </p>
      </template>
    </div>
...
import { getNoticeById } from "@/api/rentalnotice";
...
const fetchNoticeDetail = async (id) => {
  ...
  try {
    ...
    console.log(res);
    if (!res) throw new Error("공고 상세를 불러오는 데 실패했습니다.");
    notice.value = res.rentalNotice;
  } catch (e) {
